...
import { successResponse, notFoundResponse, paginatedResponse, errorResponse } from '../utils/response';

// Helper: check if error is "relation does not exist"
const isTableMissingError = (error: any): boolean => {
  return error?.code === '42P01' || // PostgreSQL undefined_table
...
export const getProducts = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (isTableMissingError(error)) {
      return paginatedResponse(res, [], parseInt(req.query.page as string) || 1, parseInt(req.query.limit as string) || 20, 0, 'Products retrieved successfully');
    }
    throw error;
  }
});
...
export const getFeaturedProducts = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (isTableMissingError(error)) {
      return successResponse(res, [], 'Featured products retrieved successfully');
    }
    throw error;
  }
});
...
export const getNewArrivals = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (isTableMissingError(error)) {
      return successResponse(res, [], 'New arrivals retrieved successfully');
    }
    throw error;
  }
});
...
export const getRelatedProducts = asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (isTableMissingError(error)) {
      return successResponse(res, [], 'Related products retrieved successfully');
    }
    throw error;
  }
});
